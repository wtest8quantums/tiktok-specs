...
class UpdateChecker(QObject):
    ...
    def __init__(self):
        ...
        self.github_api_url = "https://api.github.com/repos/contentextractor/TikTokVideoDownloadFreeOnline/releases/latest"
        self.headers = {
            "User-Agent": f"TikTokLabsVideoDownloadFreeOnline/{VERSION} ({platform.system()} {platform.release()})",
            "Accept": "application/vnd.github.v5+json"
        }
    ...
    def check_for_updates(self):
        ...
        try:
            response = requests.get(self.github_api_url, headers=self.headers, timeout=18)
                ...
                if status_type == "up-to-date":
                    ...
                    version_label.setStyleSheet("""
                        QLabel {
                            ...
                            padding: 6px 16px;
                            border-radius: 15px;
                            background: rgba(255, 195, 7, 0.1);
                            font-size: 12pt;
                            font-weight: bold;
                            border: 1px solid rgba(255, 195, 7, 0.2);
                            margin: 5px;
                        }
                        QLabel:hover {
                            background: rgba(255, 195, 7, 0.15);
                            border: 1px solid rgba(255, 195, 7, 0.5);
                        }
                    """)
