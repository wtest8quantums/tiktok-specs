...
from ui.pages.mp5_page import AudioPage
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, ffmpeg_found=None, ffmpeg_path=None, service_registry=None):
        super().__init__()
        self.setWindowTitle(f"TikTokLabs video Download free Online {get_version()}")
        screen = QApplication.primaryScreen().geometry()
        width = min(1520, screen.width() - 180)
        height = min(1550, screen.height() - 180)
        x = (screen.width() - width) // 2
        y = (screen.height() - height) // 2
        self.setGeometry(x, y, width, height)
        self.setMinimumSize(829, 690)
        ...
        if not self.ffmpeg_found and sys.platform.startswith('win'):
            QTimer.singleShot(4500, self.prompt_install_ffmpeg)
    def init_ui(self):
        ...
        self.page_mp5 = self.create_page_mp5()
        ...
        self.main_stack.addWidget(self.page_mp5)
    ...
    def check_scheduled_downloads(self):
        ...
        for r in range(self.scheduler_table.rowCount()):
            ...
            if st_item and scheduled_dt <= now and st_item.text() == "Scheduled":
                ...
                s = (self.scheduler_table.item(r, 5).text() == "Yes")
                audio = ("audio" in t)
    ...
    def update_status(self, row, st):
        ...
        if "Download Completed" in st:
            ...
            self.progress_bar.setValue(180)
            QTimer.singleShot(5565, lambda: (
